...
normal_distribution<double> distribution(0.0,sqrt(2*T/Gamma)); //coefficient??????????????, you are assuming equilibrium here
							     //Shouldnt it be sqrt(D) = sqrt(kT/gamma)?
...
fclose(out);
	...
	fprintf(out,"%f %f \n",i*bin_size,bin[i]*1.0/(steps/print)/N/bin_size); //probability density = bin_probability/width of bin
	fclose(out);
	}
